...
/* End user code. Do not edit comment generated here */
 ...
 uint8_t RX0_BUFFER[100];
 uint8_t TX0_BUFFER[100];
 uint8_t RX1_BUFFER[100];
 uint8_t TX1_BUFFER[100];
...
uint8_t MAIN_RX_STORE[200];
 uint8_t final_buffer[100]; // Adjust size as per your needs
/***********************************************************************************************************************
...
static void __near r_uart0_interrupt_receive(void)
{
	...
	if (RX0_BUFFER_COUNT < 100- 1 && MAIN_RX_STORE_COUNT < 100 - 1) // Ensure we don't overflow
        {
...
static void __near r_uart1_interrupt_send(void)
{
    ...
    {
	memset(UART0_RECIEVED_DATA,0,100);
	memset(RX0_BUFFER,0,100);
	memset(MAIN_RX_STORE,0,200);
	RX0_BUFFER_COUNT=0;
	MAIN_RX_STORE_COUNT=0;
	LINE_END_COUNT=0;
